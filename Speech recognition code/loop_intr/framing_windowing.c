 ...
 * Modified to match with Voice-Control Project
 Group 13- Mad
 *
 *****************************************************************************/
...
//static float Ya[row_length] = {0.0};//contain all mean value of each frame						
int i=0,j=0,k=0;
//float IMX = -999999999;
//float IMN = 999999999;
/* Function to detect speech and then perform Framing and Windowing */
int framing_windowing(short sample, struct buffer *real_data,int start) {
	
	int signal; /* Detect if signal is present */
  	//int ret = 0; /* Return Value */
  	if(signal_on == 0) signal = detect_envelope(block_dc(sample)); /* approx. signal envelope */
  	
  	if (start == 1) {  /* If row_index reaches row_length which is
	...
	/* Perform windowing and Framing */
			real_data->data[row_index][column_index].real = ((float)sample/reduce_gain1)*hamming_window[column_index];
		}
		if ( (column_index >= 100) && ((row_index+1) < row_length)) {
	/* Perform windowing and Framing */
			real_data->data[row_index+1][column1_index].real = ((float)sample/reduce_gain1)*hamming_window[column1_index];	
			column1_index++; /* Increment the sample for the second Frame */
		}
		if ( (column_index >= 200) && ((row_index+2) < row_length )) {
	/* Perform windowing and Framing */
			real_data->data[row_index+2][column2_index].real = ((float)sample/reduce_gain1)*hamming_window[column2_index];
...
/* Perform windowing and Framing */
	real_data->data[row_index][column_index].real = ((float)sample/reduce_gain1/reduce_gain)*hamming_window[column_index];
	column_index++; /* Increment the sample number */
    signal_on = 1;        /* start signal tracking */
   }
  return row_index; /* Return to the main program */
}
